...
interface Swaps {
  ...
  on(event: 'swap.paid', listener: (deal: SwapResult, myRole: SwapDealRole) => void): this;
  ...
  emit(event: 'swap.paid', swapResult: SwapResult, myRole: SwapDealRole): boolean;
...
class Swaps extends EventEmitter {
  ...
  private setDealPhase = (deal: SwapDeal, newPhase: SwapDealPhase): void => {
    ...
    if (deal.phase === SwapDealPhase.AmountReceived) {
      ...
      this.emit('swap.paid', swapResult, deal.myRole);
    }
  }
